...
entrypoint は `["/bin/sh", "/opt/run.sh"]` という argv 直渡しにしてあります。
`sh -c "..."` で文字列を渡す形は使いません。あの形は YAML の引用符・シェルの
引用符・Compose の変数展開が三重にかかり、実際に一度事故を起こしました
（`tr -d "\r"` の `\r` が実 CR 化 → YAML が改行をスペースに畳む →
`tr -d " "` になってスクリプトから全スペースが消える）。
渡すものが増えるほど壊れるので、argv のまま渡すのが確実です。
